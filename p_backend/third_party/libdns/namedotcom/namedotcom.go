...
func (n nameDotComRecord) RR() libdns.RR {
	return libdns.RR{
		Name: n.Host,
...
func (n *nameDotComRecord) toSanitized(libdnsRecord libdns.Record, zone string) string {
	return sanitizeName(libdnsRecord.RR().Name, zone)
}
...
func (n *nameDotComRecord) setIDFromString(id string) {
	if id == "" {
		return
	}
...
func sanitizeName(name, zone string) string {
